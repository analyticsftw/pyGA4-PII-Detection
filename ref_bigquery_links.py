...
def list_summaries(transport: str = None):
    ...
    for account_summary in results:
        for property_summary in account_summary.property_summaries:
            ...
            links = client.list_big_query_links(parent=f'{property_summary.property}')
            
            for link in links:
                print (f'"{property_summary.property}","{property_summary.display_name}","{link.name}","{link.project}","{link.daily_export_enabled}","{link.streaming_export_enabled}","{link.export_streams}","{link.excluded_events}"')
                # exit()    
